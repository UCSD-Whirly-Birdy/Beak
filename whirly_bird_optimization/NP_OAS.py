...
import openaerostruct_wb as coas
...
for x in range (0, c, c/100) # need to define c from previous optimized geometry
    ...
    indep_var_comp.add_output('cg', val=np.array[x, 0, 0], units='m')
    indep_var_comp.add_output('alpha', val = np.array[2., 2.0001])
    ...
    Cmalpha = (prob['aero_point_0.CM'][1] - prob['aero_point_0.CM'][0]) / (prob['alpha'][1] - prob['alpha'][0])
