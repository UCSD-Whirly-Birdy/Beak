...
class StabilityGroup(Group):
    ...
    def setup(self):
        ...
        comp = ExecComp('neutral_point = (chord + wing_span*np.tan(sweep*np.pi/180))/4', shape=shape)
        self.add_subsystem('neutral_point_comp', comp, promotes = ['*'])

        comp = ExecComp('center_of_gravity = ' + 
            'body_weight_ratio*chord/2 + ' +
            'wing_weight_ratio*(wing_span*np.tan(sweep*np.pi/180)/4 + chord/2) + ' + 
            'motor_weight_ratio*(wing_span*np.tan(sweep*np.pi/180)/2 + chord/2)')
